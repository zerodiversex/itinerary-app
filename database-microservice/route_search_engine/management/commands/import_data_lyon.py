...
class Command(BaseCommand):
    ...
    def import_stops(self):
        ...
        if response.status_code == 200:
            ...
            for line in lines[1:]:
                ...
                if parent_station != '':
                    stop = Stop.objects.create(stop_id=stop_id, stop_name=stop_name, stop_lat=stop_lat, stop_lon=stop_lon, parent_station=parent_station)
                else:
                    stop = Stop.objects.create(stop_id=stop_id, stop_name=stop_name, stop_lat=stop_lat, stop_lon=stop_lon)
                stop.save()
    ...
    def import_routes(self):
        ...
        if response.status_code == 200:
            ...
            for line in lines[1:]:
                ...
                route = Route.objects.create(route_id=route_id, route_short_name=route_short_name, route_long_name=route_long_name, route_type=route_type, route_desc=route_desc)
                route.save()
    ...
    def import_trips(self):
        ...
        Trip.objects.from_csv(
            ...
            dict(route='route', trip_id='trip_id', trip_headsign='trip_headsign')  # A crosswalk of model fields to CSV headers.
        )
    ...
    def import_transfer(self):
        ...
        if response.status_code == 200:
            ...
            for line in lines[1:]:
                ...
                if transfer_type == "2":
                    transfer = Transfer.objects.create(from_stop_id=from_stop_id, to_stop_id=to_stop_id, transfer_type=transfer_type, min_transfer_time=min_transfer_time)
                    transfer.save()
    ...
    def import_stop_times(self):
        ...
        StopTime.objects.from_csv("stop_times.csv", dict(stop='stop_id', trip='trip_id', arrival_time='arrival_time', departure_time='departure_time', stop_sequence='stop_sequence'))
    ...
    def import_connections(self):
        ...
        for trip_id, route_id, route_type in trip_queryset:
            print(j)
            stop_times_queryset = StopTime.objects.filter(trip=trip_id).order_by('stop_sequence').values_list('stop_id', 'arrival_time', 'departure_time')
